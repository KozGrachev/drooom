...
function PlayButton ({ instrument, shape }) {
  ...
  useEffect(() => {
    socket.on('play-instrument', (inst) => {
      if (inst === instrument) {
        ...
        setIsPlaying(Brain.instrumentState[instrument].isPlaying);
      }
    });
  }, []);

  const [isPlaying, setIsPlaying] = useState(Brain.instrumentState[instrument].isPlaying);
  ...
  return (
    <div className={`play-button ${shape}`} onMouseDown={play}>
      {
        shape === 'circle'
          ? Brain.instrumentState[instrument].isPlaying
